...
impl XaiProtoBuilder {
    ...
    fn emit_rerun_if_changed<'a>(
        ...
        // Named temporary files keep this path portable on Windows, where
        // /dev/stdout and /dev/null do not exist.
        for proto in protos {
            let dependency_file = tempfile::NamedTempFile::new()
                .context("failed to create protoc dependency output")?;
            let descriptor_file = tempfile::NamedTempFile::new()
                .context("failed to create protoc descriptor output")?;
            let dependency_path = dependency_file
                .path()
                .to_str()
                .context("dependency output path not UTF-8")?;
            let descriptor_path = descriptor_file
                .path()
                .to_str()
            ...
            command
                .arg(format!("--dependency_out={dependency_path}"))
                .arg(format!("--descriptor_set_out={descriptor_path}"));
            ...
            let prefix = format!("{descriptor_path}:");
            ...
            for line in iter::once(rem).chain(lines) {
                ...
                // Depending on absolute paths inside the protoc installation is
                // valid, but we want the output to remain deterministic.
