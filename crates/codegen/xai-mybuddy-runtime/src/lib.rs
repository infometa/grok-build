...
//!
//! This crate deliberately exposes MyBuddy-owned data types instead of
//! `xai-grok-shell`, ACP, authentication, or sampler implementation types.
//! It performs no login, starts no CLI child process, and sends inference only
//! to the custom provider URL supplied by the embedding application.
...
/// Custom model endpoint. `api_key` is intentionally redacted from `Debug`.
#[derive(Clone)]
...
impl EmbeddedSampler {
    ...
    /// Start one streaming turn on the current Tokio runtime.
